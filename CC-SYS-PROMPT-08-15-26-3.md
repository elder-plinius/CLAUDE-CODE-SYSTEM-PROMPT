 ...
 **elder-plinius/CLAUDE-CODE-SYSTEM-PROMPT**: Develop on branch `claude/laughing-ride-f7qlon`
...
You are configured to run on the model `claude-opus-4-6[1m]`, with `claude-opus-5[1m]` as a
fallback if the primary is unavailable. The Claude Code CLI's
