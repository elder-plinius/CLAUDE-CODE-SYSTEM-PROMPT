# CC-SYS-PROMPT-05-20-26 (Part 2 of 3)
...
Parameters: file_path (string, required), new_string (string, required), old_string (string, required), replace_all (boolean, default false).
...
Parameters: file_path (string, required), limit (integer), offset (integer, min 0), pages (string, PDF only).
...
Parameters: caption (string, optional), files (array of strings, required, minItems 1), status ("normal" or "proactive", required).
...
Parameters: max_results (number, default 5), query (string, required).
...
Parameters: content (string, required), file_path (string, required).
